...
When none is configured, cgvant uses the built-in default
([`compgenlab/cgvant-public-data-registry`](https://github.com/compgenlab/cgvant-public-data-registry)).
Configure your own (one or several, searched in order) in `config.toml`:

```toml
registries = [
  "https://raw.githubusercontent.com/compgenlab/cgvant-public-data-registry/main/registry.toml",
...
```sh
cgvant registry list                              # catalog: snapshots + sources (all registries)
cgvant registry pull-snapshot 2026-07             # write snapshots/2026-07.toml + all its sources
cgvant registry add-source clinvar:2026-01 --snapshot 2026-07   # add one source, ref it from a snapshot
...
- **Versions are tags** (docker-style): `clinvar:2026-01` pins a version, while bare
  ...
  cgvant doesn't auto-sort versions (semver `1.3`, dbSNP `b157`, dates aren't comparable), so
  the publisher declares which is latest.

After pulling, run `cgvant download` to fetch the actual data files / tool images (the
...
1. **Build and test it locally.** Add the source to a snapshot, `cgvant download` it, and
   confirm it annotates as expected.
2. **Submit:**

   ```sh
   cgvant registry submit clinvar:2026-01               # a data source
   cgvant registry submit vep:113                        # a tool source works too
   cgvant registry submit clinvar:2026-01 --dry-run      # preview the issue title + body
   ```
...
Submissions target the canonical repo (`compgenlab/cgvant-public-data-registry`); the
